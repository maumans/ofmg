...
function Index(props) {
    ...
    function handleDelete(id){
        confirm("Voulez-vous supprimer cette option") && Inertia.delete(route("etablissement.option.destroy",[props.auth.user.id,id]),{preserveScroll:true})
    }
    ...
    function handleSubmitEdit(){
        Inertia.put(route("etablissement.option.update",[props.auth.user.id,data.editId]),data,{preserveScroll:true});
        setOpen(false)
    }

    function handleShow(id){
        Inertia.get(route("etablissement.option.show",[props.auth.user.id,id]))
    }
    ...
    function handleSubmit(e)
    {
        ...
        post(route("etablissement.option.store",props.auth.user.id),{data,onSuccess: ()=>reset("libelle")})
    ...
    return (
        <AdminPanel auth={props.auth} error={props.error} sousActive={"option"} active={"paramètrage"}>
            <div className={"p-5"}>
                <div>
