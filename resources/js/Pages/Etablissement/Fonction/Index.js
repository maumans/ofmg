...
function Index(props) {
    ...
    function handleDelete(id){
        confirm("Voulez-vous supprimer cette fonction") && Inertia.delete(route("etablissement.fonction.destroy",[props.auth.user.id,id]),{preserveScroll:true})
    }
    ...
    function handleSubmitEdit(){
        Inertia.put(route("etablissement.fonction.update",[props.auth.user.id,data.editId]),data,{preserveScroll:true});
        setOpen(false)
    }
    ...
    function handleSubmit(e)
    {
        ...
        post(route("etablissement.fonction.store",props.auth.user.id),{data, onSuccess: ()=>reset("libelle")})
    ...
    return (
        <AdminPanel auth={props.auth} error={props.error} sousActive={"fonction"} active={"paramètrage"}>
            <div className={"p-5"}>
                <div>
