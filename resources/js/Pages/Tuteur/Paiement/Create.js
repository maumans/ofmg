...
function Create({auth,etablissement,apprenant,matricule,nbrMois,modePaiements,success,montantTotal,paiements,errors,tuteur,}) {
    ...
    {
        data.matricule && Inertia.get(route("tuteur.paiement.search",[auth?.user?.id,data.matricule]),{preserveScroll:true})
    }
            ...
            <Box sx={{ width: '100%',marginTop:10 }}>
                <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                    <Tabs value={value} onChange={handleChange} aria-label="basic tabs example" centered>
                        <Tab label="LISTE DE MES ENFANTS" {...a11yProps(0)} />
                                                ...
                                                <div className={"grid md:grid-cols-2 sm:grid-cols-2 grid-cols-1 col-span-3 gap-4"}>
                                                        {
                                                            ...
                                                            <div hidden={!apprenant?.tarifs}
                                                                 className={"md:col-span-2 sm:col-span-2 text-lg"}>
